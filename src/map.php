...
class map
{
    ...
    public $extent = [-1, -1, -1, -1];
    /** @var int Size Y (height) in pixels of the output image. */
    public $height = 500;
    /**
     * @var int[] Map background color (RGB Format).
    ...
    public $projection;
    /**
     * @var int MapFile Status (Is the map active ?).
     * @note Use :
     * * self::STATUS_ON
     * * self::STATUS_OFF
     */
    ...
    /** @var int Size X (width) in pixels of the output image. */
    public $width = 500;
     ...
     *
     * @param int $width  Width in pixels of the output image.
     * @param int $height Height in pixels of the output image.
     */
    ...
    public function save($filename)
    {
        ...
        if (!empty($this->extent)) {
            fwrite($f, '  EXTENT '.implode(' ', $this->extent).PHP_EOL);
        }
        if (!empty($this->imagecolor) && count($this->imagecolor) == 3 && array_sum($this->imagecolor) >= 0) {
            fwrite($f, '  IMAGECOLOR '.implode(' ', $this->imagecolor).PHP_EOL);
        }
        if (!empty($this->fontsetfilename)) {
            fwrite($f, '  FONTSET "'.$this->fontsetfilename.'"'.PHP_EOL);
        }
        if (!empty($this->symbolsetfilename)) {
            fwrite($f, '  SYMBOLSET "'.$this->symbolsetfilename.'"'.PHP_EOL);
        }
        if (!empty($this->width) && !empty($this->height)) {
            fwrite($f, '  SIZE '.$this->width.' '.$this->height.PHP_EOL);
        }
        if (!is_null($this->units)) {
            fwrite($f, '  UNITS '.self::convertUnits($this->units).PHP_EOL);
        }
    ...
    private function read($mapfile)
    {
        ...
        while (($_sz = fgets($h, 1024)) !== false) {
            ...
            if (preg_match('/^MAP$/i', $sz)) {
                $map = true;
            } elseif ($map && is_null($reading) && preg_match('/^END( # MAP)?$/i', $sz)) {
                $map = false;
            } elseif ($map && is_null($reading) && preg_match('/^OUTPUTFORMAT$/i', $sz)) {
                $reading = 'OUTPUTFORMAT';
            } elseif ($map && $reading == 'OUTPUTFORMAT' && preg_match('/^END( # OUTPUTFORMAT)?$/i', $sz)) {
                $reading = null;
            } elseif ($map && $reading == 'OUTPUTFORMAT') {
                continue;
            } elseif ($map && is_null($reading) && preg_match('/^QUERYMAP$/i', $sz)) {
                $reading = 'QUERYMAP';
            } elseif ($map && $reading == 'QUERYMAP' && preg_match('/^END( # QUERYMAP)?$/i', $sz)) {
                $reading = null;
            } elseif ($map && $reading == 'QUERYMAP') {
                continue;
            } elseif ($map && is_null($reading) && preg_match('/^PROJECTION$/i', $sz)) {
                $reading = 'PROJECTION';
            } elseif ($map && $reading == 'PROJECTION' && preg_match('/^END( # PROJECTION)?$/i', $sz)) {
                $reading = null;
            } elseif ($map && $reading == 'PROJECTION' && preg_match('/^"init=(.+)"$/i', $sz, $matches)) {
                $this->projection = $matches[1];
            } elseif ($map && is_null($reading) && preg_match('/^LEGEND$/i', $sz)) {
                $reading = 'LEGEND';
                $legend[] = $sz;
            } elseif ($map && $reading == 'LEGEND' && preg_match('/^LABEL$/i', $sz)) {
                $legend[] = $sz;
                $reading = 'LEGEND_LABEL';
            } elseif ($map && $reading == 'LEGEND' && preg_match('/^END( # LEGEND)?$/i', $sz)) {
                $legend[] = $sz;
                $this->legend = new Legend($legend);
                $reading = null;
                unset($legend);
            } elseif ($map && $reading == 'LEGEND') {
                $legend[] = $sz;
            } elseif ($map && $reading == 'LEGEND_LABEL' && preg_match('/^END( # LABEL)?$/i', $sz)) {
                $legend[] = $sz;
                $reading = 'LEGEND';
            } elseif ($map && $reading == 'LEGEND_LABEL') {
                $legend[] = $sz;
            } elseif ($map && is_null($reading) && preg_match('/^SCALEBAR$/i', $sz)) {
                $reading = 'SCALEBAR';
                $scalebar[] = $sz;
            } elseif ($map && $reading == 'SCALEBAR' && preg_match('/^LABEL$/i', $sz)) {
                $scalebar[] = $sz;
                $reading = 'SCALEBAR_LABEL';
            } elseif ($map && $reading == 'SCALEBAR' && preg_match('/^END( # SCALEBAR)?$/i', $sz)) {
                $scalebar[] = $sz;
                $this->scalebar = new Scalebar($scalebar);
                $reading = null;
                unset($scalebar);
            } elseif ($map && $reading == 'SCALEBAR') {
                $scalebar[] = $sz;
            } elseif ($map && $reading == 'SCALEBAR_LABEL' && preg_match('/^END( # LABEL)?$/i', $sz)) {
                $scalebar[] = $sz;
                $reading = 'SCALEBAR';
            } elseif ($map && $reading == 'SCALEBAR_LABEL') {
                $scalebar[] = $sz;
            } elseif ($map && is_null($reading) && preg_match('/^LAYER$/i', $sz)) {
                $reading = 'LAYER';
                $layer[] = $sz;
            } elseif ($map && $reading == 'LAYER' && preg_match('/^PROJECTION$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER_PROJECTION';
            } elseif ($map && $reading == 'LAYER' && preg_match('/^CLASS$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER_CLASS';
            } elseif ($map && $reading == 'LAYER' && preg_match('/^METADATA$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER_METADATA';
            } elseif ($map && $reading == 'LAYER' && preg_match('/^VALIDATION$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER_VALIDATION';
            } elseif ($map && $reading == 'LAYER' && preg_match('/^END( # LAYER)?$/i', $sz)) {
                $layer[] = $sz;
                $this->addLayer(new Layer($layer));
                $reading = null;
                unset($layer);
            } elseif ($map && $reading == 'LAYER') {
                $layer[] = $sz;
            } elseif ($map && $reading == 'LAYER_PROJECTION' && preg_match('/^END( # PROJECTION)?$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER';
            } elseif ($map && $reading == 'LAYER_PROJECTION') {
                $layer[] = $sz;
            } elseif ($map && $reading == 'LAYER_CLASS' && preg_match('/^END( # CLASS)?$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER';
            } elseif ($map && $reading == 'LAYER_CLASS') {
                $layer[] = $sz;
            } elseif ($map && $reading == 'LAYER_METADATA' && preg_match('/^END( # METADATA)?$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER';
            } elseif ($map && $reading == 'LAYER_METADATA') {
                $layer[] = $sz;
            } elseif ($map && $reading == 'LAYER_VALIDATION' && preg_match('/^END( # VALIDATION)?$/i', $sz)) {
                $layer[] = $sz;
                $reading = 'LAYER';
            } elseif ($map && $reading == 'LAYER_VALIDATION') {
                $layer[] = $sz;
            } elseif ($map && is_null($reading) && preg_match('/^WEB$/i', $sz)) {
                $reading = 'WEB';
            } elseif ($map && $reading == 'WEB' && preg_match('/^METADATA$/i', $sz)) {
                $reading = 'WEB_METADATA';
            } elseif ($map && $reading == 'WEB' && preg_match('/^END( # WEB)?$/i', $sz)) {
                $reading = null;
            } elseif ($map && $reading == 'WEB_METADATA' && preg_match('/^END( # METADATA)?$/i', $sz)) {
                $reading = null;
            } elseif ($map && $reading == 'WEB_METADATA' && preg_match('/^"(.+)"\s"(.+)"$/i', $sz, $matches)) {
                $this->metadata[$matches[1]] = $matches[2];
            } elseif ($map && is_null($reading) && preg_match('/^NAME "(.+)"$/i', $sz, $matches)) {
                $this->name = $matches[1];
            } elseif ($map && is_null($reading) && preg_match('/^STATUS (.+)$/i', $sz, $matches)) {
                $this->status = self::convertStatus(strtoupper($matches[1]));
            } elseif ($map && is_null($reading) && preg_match('/^EXTENT (-?[0-9\.]+) (-?[0-9\.]+) (-?[0-9\.]+) (-?[0-9\.]+)$/i', $sz, $matches)) {
                $this->extent = [$matches[1], $matches[2], $matches[3], $matches[4]];
            } elseif ($map && is_null($reading) && preg_match('/^FONTSET "(.+)"$/i', $sz, $matches)) {
                $this->fontsetfilename = $matches[1];
            } elseif ($map && is_null($reading) && preg_match('/^SYMBOLSET "(.+)"$/i', $sz, $matches)) {
                $this->symbolsetfilename = $matches[1];
            } elseif ($map && is_null($reading) && preg_match('/^SIZE ([0-9]+) ([0-9]+)$/i', $sz, $matches)) {
                $this->size = [$matches[1], $matches[2]];
            } elseif ($map && is_null($reading) && preg_match('/^UNITS (.+)$/i', $sz, $matches)) {
